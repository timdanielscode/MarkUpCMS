...
class Post extends Model {
    ...
    public function allPostsWithCategories($searchValue = null) {
        ...
        if(!empty($searchValue) && $searchValue !== null) {
            ...
            return DB::try()->select('pages.id, pages.title, pages.slug, pages.author, pages.metaTitle, pages.metaDescription, pages.removed, pages.date_created_at, pages.date_updated_at, pages.time_created_at, pages.time_updated_at, categories.title')->from('pages')->joinLeft('category_page')->on('category_page.page_id', '=', 'pages.id')->joinLeft('categories')->on('categories.id', '=', 'category_page.category_id')->where('pages.removed', 'IS', NULL)->or('pages.removed', '=', '0')->fetch();
        }
    }
}